    ...
    Object.keys(c_board).forEach(val => {
        if (c_board[val].piece !== null) {
            if (c_board[val].piece.piece_name === "Wking" || c_board[val].piece.piece_name === "Bking"){
                //Do nothing
            } else if (Object.keys(can_block_check(color)).includes(val)) {
                c_board[val].piece.moves = (can_block_check(color))[val]
            } else if (color !== c_board[val].piece.piece_color){
                //Do nothing
            } else {
                c_board[val].piece.moves = []
            }
        }
    });
}
...
function checking_direction(checking_square: string, color: string) {
    ...
    else if (first_char(checking_square_int) === first_char(king_square_int) || (checking_square_int  <= 8 && king_square_int <= 8)) {
...
function check_check() {
    ...
    Object.keys(c_board).every(id => {
        ...
        } else if (c_board[id].piece !== null && (["Bpawn", "Wpawn"].includes(c_board[id].piece.piece_name) && c_board[id].piece.pawn_moves.includes(slot().id))) {
            if (color === "white"){
                update_check(id, color, false)
                return false
            } else {
    ...
    Object.keys(c_board).forEach(id => {
        if (c_board[id].piece !== null && slot().checking_piece.piece.piece_color !== c_board[id].piece.piece_color 
            ...
            || c_board[id].piece.moves.includes(slot().checking_piece.id))) {
                disrupt_moves[id] = ((c_board[id].piece.moves.filter(id => {return id === slot().checking_piece.id || in_checking_dir(color, parseInt(slot().checking_piece.id)).includes(id)})))
        }
    });
    return disrupt_moves
}
